    ...
    <main class="horse-racing__main">
      ...
      <aside class="horse-racing__col horse-racing__col--right">
        <div class="horse-racing__panels">
          <RoundPanel title="Program" :items="programItems" :active-id="currentRound?.id" />
          <RoundPanel title="Results" :items="resultItems" />
        </div>
      </aside>
      <MobileOverlay
        v-if="isMenuOpen"
        :active-slide="activeOverlaySlide"
        @slide-change="activeOverlaySlide = $event"
      />
    </main>
  </div>
</template>
...
import MobileOverlay from '@/features/horse-racing/components/MobileOverlay.vue'
...
const activeOverlaySlide = ref(0)
